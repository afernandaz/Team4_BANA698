...
## ============================================================
## HELPER: ONE-WAY ANOVA + GROUP MEANS  (H1 - H2)
## ============================================================
...
## ============================================================
## H2a: Content design configuration predicts early like rate
## ============================================================
print("\n\n" + "#"*60)
print("# HYPOTHESIS 2a (H2a)")
print("# H2a: Content design configuration significantly predicts")
...
## ============================================================
## H2b: Content design configuration predicts early share rate
## ============================================================
print("\n\n" + "#"*60)
print("# HYPOTHESIS 2b (H2b)")
print("# H2b: Content design configuration significantly predicts")
...
## ============================================================
## H2c: Content design configuration predicts early comment rate
## ============================================================
print("\n\n" + "#"*60)
print("# HYPOTHESIS 2c (H2c)")
print("# H2c: Content design configuration significantly predicts")
...
## ============================================================
## POST-HOC: TUKEY HSD PAIRWISE COMPARISONS (H1 - H2)
## Runs after each ANOVA to show WHICH specific configuration
## pairs are significantly different from each other.
## ANOVA only tells you that *some* groups differ — Tukey tells
## you *which* ones.
## ============================================================

def run_tukey(outcome_col, outcome_label):
    print(f"\n{'='*60}")
    print(f"  Tukey HSD Post-Hoc: {outcome_label}")
    print(f"{'='*60}")

    # Drop any missing values before running
    clean = df[[outcome_col, 'content_design_config']].dropna()

    tukey = pairwise_tukeyhsd(
        endog=clean[outcome_col],
        groups=clean['content_design_config'],
        alpha=0.05
    )

    # Pull results into a dataframe for clean printing
    result_df = pd.DataFrame(
        data=tukey._results_table.data[1:],
        columns=tukey._results_table.data[0]
    )
    result_df.columns = ['Group1', 'Group2', 'MeanDiff', 'p-adj', 'Lower', 'Upper', 'Reject']

    # Show significant pairs first, then by size of difference
    result_df['sig'] = result_df['Reject'].astype(str).str.lower() == 'true'
    result_df['absdiff'] = result_df['MeanDiff'].astype(float).abs()
    result_df = result_df.sort_values(['sig', 'absdiff'], ascending=[False, False])

    print(f"\n{'Group 1':<30} {'Group 2':<30} {'Mean Diff':>10} {'p-adj':>8}  Sig?")
    print("-" * 88)
    for _, row in result_df.iterrows():
        sig_label = "YES *" if row['sig'] else "no"
        print(f"{str(row['Group1']):<30} {str(row['Group2']):<30} "
              f"{float(row['MeanDiff']):>10.4f} {float(row['p-adj']):>8.4f}  {sig_label}")

    sig_count = result_df['sig'].sum()
    total     = len(result_df)
    print(f"\n  → {sig_count} of {total} pairwise comparisons significant at p < .05")


print("\n\n" + "#"*60)
print("# POST-HOC TUKEY HSD — ALL ANOVA OUTCOMES")
print("# Identifies which specific configuration pairs differ")
print("#"*60)

run_tukey('early_engagement_velocity', 'Engagement Velocity (H1)')
run_tukey('early_like_rate',           'Early Like Rate (H2a)')
run_tukey('early_share_rate',          'Early Share Rate (H2b)')
run_tukey('early_comment_rate',        'Early Comment Rate (H2c)')


## ============================================================
## HELPER: LINEAR REGRESSION WITH MODERATION  (H3 - H4)
...
## ============================================================
## H3: Authority log MODERATES content design → engagement
## ============================================================
print("\n\n" + "#"*60)
print("# HYPOTHESIS 3 (H3)")
print("# H3: Influencer authority log moderates the relation between")
...
## ============================================================
## H4: Verified status MODERATES content design → engagement
## ============================================================
print("\n\n" + "#"*60)
print("# HYPOTHESIS 4 (H4)")
print("# H4: Influencer verification status moderates the relation")
